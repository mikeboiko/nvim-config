...
- `<leader>ag` saves buffers, stages all changes silently, asks CopilotChat for a Conventional Commit message, and passes it to `~/git/Linux/git/gap -m` to commit, rebase, and push without a second AI call. It shows one final success or failure notification; failed runs keep their full output open in the terminal split, while successful splits close automatically. CopilotChat closes when the gap process finishes. The shared `git/gap` script strips Copilot co-author trailers from commit messages.
