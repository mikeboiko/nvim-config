...
- Completion and AI tooling with `blink.cmp`, `copilot.lua`, `blink-cmp-copilot`, and `CopilotChat.nvim`; chat defaults to GPT-6 Luna with maximum reasoning effort.
...
- `<leader>ag` saves buffers, stages all changes, asks CopilotChat for a Conventional Commit message, and passes it to `~/git/Linux/git/gap -m` to commit, rebase, and push without a second AI call. The workflow runs in a visible terminal split; failed runs keep their full output open and also show an error notification. Successful gap terminal splits close automatically, and CopilotChat closes when the gap process finishes. The shared `git/gap` script strips Copilot co-author trailers from commit messages.
