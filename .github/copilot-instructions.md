...
- Reusable migrated legacy helpers are increasingly landing in focused modules under `lua/config/`, with `windows.lua`, `quickfix.lua`, `folds.lua`, `comments.lua`, `terminal.lua`, `clipboard.lua`, `tabline.lua`, `editor.lua`, `shell.lua`, and `buffers.lua` as current examples.
